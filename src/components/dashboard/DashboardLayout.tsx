...
const DashboardLayout: FC<IProps> = ({ children }) => {
    ...
    // Auto-check for updates on startup if the user has opted in (skip in dev mode)
    useEffect(() => {
    ...
    return (
        <main className="min-h-screen p-10 max-w-5xl mx-auto flex flex-col gap-8">
            ...
            {updateAvailable && !installingUpdate && (
                <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 px-5 py-3 rounded-2xl bg-neutral-800 border border-white/10 backdrop-blur-sm shadow-lg animate-in fade-in slide-in-from-bottom-2 duration-300">
                    ...
                    <span className="text-sm text-white/80 font-medium">Update available — v{updateAvailable}</span>
                    <button
            ...
            {installingUpdate && (
                <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex items-center gap-3 px-5 py-3 rounded-2xl bg-neutral-800 border border-white/10 backdrop-blur-sm shadow-lg">
